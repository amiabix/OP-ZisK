...
impl KzgProof {
    /// Variable-base multi-scalar multiplication (MSM).
    ///
    /// The upstream `bls12_381` crate does not provide SP1's `msm_variable_base` extension.
    /// This implementation is correct but not optimized.
    fn msm_variable_base(points: &[G1Projective], scalars: &[Scalar]) -> G1Projective {
        assert_eq!(points.len(), scalars.len(), "msm inputs must have equal length");
    ...
    pub fn verify_kzg_proof_batch(
        ...
        let proof_lincomb = Self::msm_variable_base(&proofs, &r_powers);
        ...
        let proof_z_lincomb = Self::msm_variable_base(&proofs, &r_times_z);
        let c_minus_y_lincomb = Self::msm_variable_base(&c_minus_y, &r_powers);
