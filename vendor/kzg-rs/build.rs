...
fn main() {
    ...
    pub fn load_trusted_setup_file_brute() -> Result<KzgSettingsOwned, KzgError> {
        ...
        _g1_points.iter().enumerate().for_each(|(i, bytes)| {
            // Use checked decompression with the upstream `bls12_381` crate.
            // This is executed at build time (host), not inside the zkVM.
            g1_points[i] = Option::<G1Affine>::from(G1Affine::from_compressed(bytes))
        ...
        _g2_points.iter().enumerate().for_each(|(i, bytes)| {
            // Use checked decompression with the upstream `bls12_381` crate.
            // This is executed at build time (host), not inside the zkVM.
            g2_points[i] = Option::<G2Affine>::from(G2Affine::from_compressed(bytes))
